...
class EncoderGearFormer(nn.Module):
    def __init__(self, input_size, output_size, depth):
        super(EncoderGearFormer, self).__init__()
...
def train_gearformer(input_data, target_tensor, target_length_seq, output_size, encoder, decoder, encoder_optimizer, decoder_optimizer, weight_c, adaptive_weight, loss_weight):
    encoder.train()
    decoder.train()
...
def val_gearformer(input_data, target_tensor, target_length_seq, output_size, encoder, decoder, weight_c, adaptive_weight, loss_weight):
    encoder.eval()
    decoder.eval()
    ignore_index=output_size-1
